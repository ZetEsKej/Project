...
Lifebar::Lifebar(Ant* ant) {												//konstruktor 
...
Lifebar::Lifebar(Castle* castle) {										//konstruktor 
...
Lifebar::~Lifebar()													//destruktor
	{}

void Lifebar::updateLifebar() {								//aktualzacja paska zdrowia
...
void Lifebar::updateLifebar(Castle* castle) {							//aktualzacja paska zdrowia
		rect.setSize(sf::Vector2f(0.005f * castle->hp, 5.0f));
		rect.setPosition(castle->rect.getPosition().x + 50, castle->rect.getPosition().y + castle->rect.getSize().y + 20);		// umieszczenie paska pod zamkiem
