...
Main_menu::Main_menu(float width, float hight) {
	...
	settingsMenu[0].setFillColor(sf::Color::Red);
	settingsMenu[0].setString("Choose background");

	settingsMenu[1].setFont(font);
	settingsMenu[1].setFillColor(sf::Color::Red);
	settingsMenu[1].setString("Choose audio");
...
void Main_menu::draw_settings(sf::RenderWindow& window_sett) {
	for (int j = 0; j < 3; j++) {
		window_sett.draw_settings(settingsMenu[j]);
	}
}
